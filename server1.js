...
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!req.file) {
      // If file was not processed by multer, send 400
      return res.status(400).json({ error: "No file found in request" });
    }

    // Optionally log the file info
    console.log("Received file:", req.file.originalname);

    // ... your Google Drive upload logic ...

    console.log("File uploaded successfully to Drive");
    res.json({ message: "Success" });
  } catch (error) {
    // Important to log both message and stack
    console.error("Error in /upload route:", error.message, error.stack);
